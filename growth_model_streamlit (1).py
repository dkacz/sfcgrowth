...
def run_model(custom_params=None):
    ...
    if custom_params:
        ...
        # --- Debug Logging: Check initial state before first solve --- 
        logging.debug(f"Initial state keys before loop: {sorted(list(model.current_solution.keys()))}")
        logging.debug(f"Initial state Sk value: {model.current_solution.get('Sk', 'NOT FOUND')}")
        # --- End Debug Logging ---
